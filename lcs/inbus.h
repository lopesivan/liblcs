...
public:
    ...
    int width(void) const { return *size; }
    ...
    /// to the state of the line at index \p i. An \p OutOfRangeException is thrown if
    /// the index value \p i is beyond the bus width. The returned value is only a copy of
    /// the line state.
    ...
    /// Writes the decimal value of the binary line values into an <b> \p int <b>
    /// variable.
    ///
    /// \param intVal The decimal equivalent of the line states is written into this
    /// argument.
    ///
    ...
    /// width, then, only the lines within range are assigned lines of the new bus object.
    ///
    /// \param w The width of the part-bus.
    /// \param s The start bit from where the part-bus should be accumulated.
    template <int w>
    const InputBus<w> partBus(int s) const;

    /// An overloaded operator to obtain a single line bus of type lcs::InputBus
    /// which has one of the lines (inidicated using the parameter \p index) of the
    /// original bus.
    const InputBus<1> operator()(int index) const;
...
template <int bits>
template <int w>
const InputBus<w> InputBus<bits>::partBus(int s) const
{
    InputBus<w> b;
...
template <int bits>
const InputBus<1> InputBus<bits>::operator()(int i) const
{
    return this->partBus<1>(i);
}

template <int bits>
const LineState InputBus<bits>::operator[](int index) const throw(OutOfRangeException<int>)
{
...
template <int bits>
void InputBus<bits>::operator>>(int &intVal) const
{
    intVal = 0;
    ...
    for (int i = bits-1; i >= 0; i--)
    {
        ...
        if (val == HIGH)
            bitVal = 1;
        else if (val == UNKNOWN || val == HIGH_IMPEDENCE)
            bitVal = -100000;

        intVal = 2*intVal + bitVal;
    }
}
