...
/// A template class which encapsulates a logical NAND gate. The template parameter
...
public:
    ...
    /// Here, a NAND operation is performed on the line states of the data lines of the
...
template <int width>
Nand<width>::Nand(const Bus<1> &output, const InputBus<width> &input)
...
template <int width>
void Nand<width>::propogate(void)
{
