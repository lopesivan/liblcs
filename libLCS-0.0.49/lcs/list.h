...
public:
    ...
    /// In case of an object with multiple references is
    /// pop-ed out to empty-ness, then the multiple references would no longer hold. Each
    /// individual object would have its own data in memory on subsequent refilling of the object.
    void pop(void);
    ...
    /// Removes all list elements equal to \p e.
    ///
    void remove(T e);

    /// Destroys the list.
    /// All the memory held by the list is destroyed. Care has to be taken in using this method
    /// when the list data has multiple references.
    /// In case of an object with multiple references is
    /// destroyed to empty-ness, then the multiple references would no longer hold. Each
    /// individual object would have its own data in memory on subsequent refilling of the object.
    void destroy(void);
...
{
    ListNode<T> *nodePtr = dataPtr->node;
    ...
    if (nodePtr->previousNodePtr == NULL && nodePtr->nextNodePtr == NULL)
    {
        delete dataPtr->node;
        dataPtr->node = NULL;
        (*size)--;
    }
    else if (nodePtr->previousNodePtr == NULL)
    {
        dataPtr->node = (dataPtr->node)->nextNodePtr;
        dataPtr->node->previousNodePtr = NULL;

        delete nodePtr;
        (*size)--;
    }
    else if (nodePtr->nextNodePtr == NULL)
    {
        lastNodePtr = lastNodePtr->previousNodePtr;
        lastNodePtr->nextNodePtr = NULL;
    ...
    {
        (nodePtr->nextNodePtr)->previousNodePtr = nodePtr->previousNodePtr;
        (nodePtr->previousNodePtr)->nextNodePtr = nodePtr->nextNodePtr;
