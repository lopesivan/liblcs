...
/// A template class specialisation which encapsulates a expressions consisting of single
/// lines. A user of libLCS will never need to use this class explicitly. Implicitly, this
/// class is used everytime a bitselect operation is performed on bus objects.
template <>
...
public:
    ...
    /// The only usefull constructor. The default constructor is practically useless.
    ///
    Expression(Line *line) : exprl(line) {}

    /// Copy constructor.
    ///
    Expression(const Expression<1, LINE_EXPR, void, void> &expr): exprl(expr.exprl) {}

    /// This function can be used by a \p lcs::Module derivative to be notified of line
    /// state changes in the busses used in the expression.
    void notify(Module *mod) { exprl->notify(LINE_STATE_CHANGE, mod, 0); }

    /// Returns the bit state (which is a result of the operation performed by the
    /// expression) at the index specified. In the current context, 0 is the only
    /// allowed index value!
    const LineState operator[](int index) const throw (OutOfRangeException<int>)
    {
        if (index != 0)
            throw OutOfRangeException<int>(0, 0, index);
    ...
    ///
    void operator=(const Expression<1, LINE_EXPR, void, void> &expr)
    {
        *exprl = expr[0];
    }

    /// Assignment operator for assignment with a \p lcs::LineState value.
    ///
    ...
    /// line-expression on the LHS with a delay-expression pair in the RHS. The value of
    ...
    {
        exprl->setAssDelay(dep.getDelay());
    ...
    ///
    /// In the second line of the above code snippet, an assignment is being made to the
    /// line-expression on the LHS with a delay-state pair in the RHS. The value of
    /// delay used here is 5 system time units. Note the use of the <b> neccessary </b>
    /// parentheses enclosing the delay-state pair.
    void operator=(const DelayStatePair &dsp)
    {
        exprl->setAssDelay(dsp.getDelay());
        *exprl = dsp.getState();
    }

    /// Returns the width for which an \p lcs::Expression object is valid. In the
    /// current context 1 is returned always!
